...
import { Send, User, Mail, Phone, CheckCircle2, AlertCircle, Briefcase } from 'lucide-react';
...
export default function LandingPageContact() {
  ...
  const [form, setForm] = useState({ nombre: '', negocio: '', email: '', telefono: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/contact', {
        ...
        body: JSON.stringify({ ...form, rubro: `Landing Page — ${form.negocio}` }),
      });
      const data = await res.json();
      if (!res.ok || !data.ok) throw new Error(data.error ?? 'Error desconocido');
      setIsSuccess(true);
      setForm({ nombre: '', negocio: '', email: '', telefono: '' });
    } catch (err: unknown) {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <div className="relative"><div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none"><User className="h-5 w-5 text-slate-500" /></div>
                      <input type="text" id="nombre" required value={form.nombre} onChange={handleChange} className={inputClasses} placeholder="Ej. Juan Pérez" /></div>
                  </div>
                  <div className="space-y-2">
                    <label htmlFor="negocio" className="text-sm font-medium text-slate-300 ml-1">Tu Negocio / Empresa</label>
                    <div className="relative"><div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none"><Briefcase className="h-5 w-5 text-slate-500" /></div>
                      <input type="text" id="negocio" required value={form.negocio} onChange={handleChange} className={inputClasses} placeholder="Ej. Mi Emprendimiento" /></div>
                  </div>
                  <div className="space-y-2">
